...
class NSIControllerSpec extends TestSupport with AkkaMaterializerSpec {
  ...
  val testCreateAccount = NSIPayload(
    ...
    generator.nextNino.nino, // scalastyle:ignore magic.number
  ...
  "Get /nsi-services/account" should {
    "return a successful status when given an existing nino" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        nsiController.getAccount(Some("correlationId"), Some(ninoWithAccount), Some("V1.0"), Some("systemId"))(request)
      status(result) shouldBe OK
    ...
    "return a 400 with errorMessageId HTS-API015-002 when the service version is missing" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        await(nsiController.getAccount(Some("correlationId"), Some(ninoWithAccount), None, Some("systemId"))(request))
      status(result)          shouldBe BAD_REQUEST
    ...
    "return a 400 with errorMessageId HTS-API015-003 when given an unsupported service version" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result = await(
        nsiController.getAccount(Some("correlationId"), Some(ninoWithAccount), Some("V1.5"), Some("systemId"))(request))
      status(result)          shouldBe BAD_REQUEST
    ...
    "return a 400 with errorMessageId HTS-API015-005 when given a nino in the incorrect format" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result = await(
        nsiController.getAccount(Some("correlationId"), Some("not-a-nino"), Some("V1.0"), Some("systemId"))(request))
      status(result)          shouldBe BAD_REQUEST
    ...
    "return a 400 with errorMessageId HTS-API015-006 when given a nino not found" in {
      ...
      val result = await(
        nsiController
          .getAccount(Some("correlationId"), Some(generator.nextNino.nino), Some("V1.0"), Some("systemId"))(request))
      status(result)          shouldBe BAD_REQUEST
    ...
    "return a 500 when given a nino with 500 in" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        nsiController.getAccount(Some("correlationId"), Some(ninoContaining500), Some("V1.0"), Some("systemId"))(
          request)
    ...
    "return a 401 when given a nino with 401 in" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        nsiController.getAccount(Some("correlationId"), Some(ninoContaining401), Some("V1.0"), Some("systemId"))(
          request)
      status(result) shouldBe UNAUTHORIZED
    }
    ...
    "return a 400 with errorMessageId HTS-API015-012 when systemId is not present" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        await(nsiController.getAccount(Some("correlationId"), Some(ninoWithAccount), Some("V1.0"), None)(request))
      status(result)          shouldBe BAD_REQUEST
  ...
  "Get /nsi-services/transactions" should {
    "return a successful status when given an existing nino" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        nsiController.getTransactions(Some("correlationId"), Some(ninoWithAccount), Some("V1.0"), Some("systemId"))(
          request)
      status(result) shouldBe OK
      val json = contentAsString(result)
      Json.fromJson[NSIGetTransactionsByNinoResponse](Json.parse(json)).get shouldBe NSIGetTransactionsByNinoResponse
        .bethResponse(Some("correlationId"))
    }
    ...
    "return a 400 with errorMessageId HTS-API015-002 when the service version is missing" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result = await(
        nsiController.getTransactions(Some("correlationId"), Some(ninoWithAccount), None, Some("systemId"))(request))
      status(result)          shouldBe BAD_REQUEST
      errorMessageIds(result) shouldBe List("HTS-API015-002")
      correlationIds(result)  shouldBe "correlationId"
    }
    ...
    "return a 400 with errorMessageId HTS-API015-003 when given an unsupported service version" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result = await(
        nsiController.getTransactions(Some("correlationId"), Some(ninoWithAccount), Some("V1.5"), Some("systemId"))(
          request))
      status(result)          shouldBe BAD_REQUEST
      errorMessageIds(result) shouldBe List("HTS-API015-003")
      correlationIds(result)  shouldBe "correlationId"
    ...
    "return a 400 with errorMessageId HTS-API015-004 when not given a nino" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        await(nsiController.getTransactions(Some("correlationId"), None, Some("V1.0"), Some("systemId"))(request))
      status(result)          shouldBe BAD_REQUEST
      errorMessageIds(result) shouldBe List("HTS-API015-004")
      correlationIds(result)  shouldBe "correlationId"
    }
    ...
    "return a 400 with errorMessageId HTS-API015-005 when given a nino in the incorrect format" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result = await(
        nsiController.getTransactions(Some("correlationId"), Some("EZ00000A"), Some("V1.0"), Some("systemId"))(request))
      status(result)          shouldBe BAD_REQUEST
      errorMessageIds(result) shouldBe List("HTS-API015-005")
      correlationIds(result)  shouldBe "correlationId"
    }
    ...
    "return a 400 with errorMessageId HTS-API015-006 when given a nino not found" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result = await(nsiController
        .getTransactions(Some("correlationId"), Some(generator.nextNino.nino), Some("V1.0"), Some("systemId"))(request))
      status(result)          shouldBe BAD_REQUEST
      errorMessageIds(result) shouldBe List("HTS-API015-006")
      correlationIds(result)  shouldBe "correlationId"
    }
    ...
    "return a 500 when given a nino with 500 in" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        nsiController.getTransactions(Some("correlationId"), Some(ninoContaining500), Some("V1.0"), Some("systemId"))(
          request)
      status(result) shouldBe INTERNAL_SERVER_ERROR
    }
    ...
    "return a 401 when given a nino with 401 in" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        nsiController.getTransactions(Some("correlationId"), Some(ninoContaining401), Some("V1.0"), Some("systemId"))(
          request)
      status(result) shouldBe UNAUTHORIZED
    }
    ...
    "return a 400 with errorMessageId HTS-API015-012 when systemId is not present" in {
      ...
      val result = await(
        nsiController
          .getTransactions(Some("correlationId"), Some(generator.nextNino.nino), Some("V1.0"), None)(request))
      status(result)          shouldBe BAD_REQUEST
      errorMessageIds(result) shouldBe List("HTS-API015-012")
      correlationIds(result)  shouldBe "correlationId"
    }
    ...
    "return a 400 with two error responses when systemId is not present and the nino is in the incorrect format" in {
      val request = FakeRequest()
        .withHeaders(authHeader)
      val result =
        await(nsiController.getTransactions(Some("correlationId"), Some("not-a-nino"), Some("V1.0"), None)(request))
      status(result)          shouldBe BAD_REQUEST
      errorMessageIds(result) shouldBe List("HTS-API015-005", "HTS-API015-012")
      correlationIds(result)  shouldBe "correlationId"
    }
